<div style="display:flex;flex-direction:column;gap:24px;height:100%;">
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <p class="txt-muted" style="font-size:13px;margin-top:4px;">Kelola tarif biaya sekolah yang tampil dinamis pada rincian biaya PPDB.</p>
        </div>
            ...
            <table class="w-full text-left" style="border-collapse:separate;border-spacing:0;">
                <thead>
                    <tr class="bg-indigo-500/5 dark:bg-white/5 border-b border-indigo-500/10 dark:border-white/10">
                        ...
                        <th class="px-5 py-4 text-[11px] font-bold uppercase tracking-wider txt-muted text-center w-28">Status</th>
                        ...
                        <tr class="hover:bg-indigo-500/[0.02] dark:hover:bg-white/[0.02] transition-colors group">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        @if($item->jurusan)
                                            <p class="mt-0.5 text-xs font-semibold text-cyan-400">{{ $item->jurusan->kode }} · {{ $item->jurusan->nama }}</p>
                                        @endif
                                        @if($item->keterangan)
                            ...
                            <td class="px-5 py-4 text-center">
                                <button wire:click="toggleActive({{ $item->id }})"
                                    class="inline-flex items-center rounded-lg border px-2.5 py-1 text-xs font-bold transition-colors cursor-pointer {{ $item->is_active ? 'border-emerald-500/20 bg-emerald-500/10 text-emerald-400' : 'border-slate-500/20 bg-slate-500/10 text-slate-400' }}"
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-20 text-center">
                                <div class="flex flex-col items-center justify-center opacity-40">
                                    ...
                                    <p class="text-xs txt-muted mt-1">Klik "Tambah Biaya" untuk memulai.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    <x-ui.modal name="spp-form" :show="$isModalOpen" maxWidth="lg">
        <div class="py-1">
            <h2 class="text-xl font-bold txt-primary mb-1">
                {{ $editId ? 'Edit Biaya Sekolah' : 'Tambah Biaya Sekolah' }}
            </h2>
            ...
            <form wire:submit.prevent="save" class="space-y-4">
                ...
                @if($jenjang === 'SMK')
                    <div>
                        @php
                            $jurusanOptions = ['' => 'Umum untuk semua jurusan SMK'];
                            foreach ($jurusans as $jurusan) {
                                $jurusanOptions[$jurusan->id] = $jurusan->kode . ' - ' . $jurusan->nama;
                            }
                        @endphp
                        <x-ui.select label="Jurusan SMK" wire:model="jurusan_id" :options="$jurusanOptions" />
                        <p class="mt-1 text-xs txt-muted">Pilih jurusan jika tarif hanya berlaku untuk jurusan tersebut.</p>
                        @error('jurusan_id') <p class="mt-1 text-xs text-red-400">{{ $message }}</p> @enderror
                    </div>
                @endif
                ...
                <div class="flex items-start gap-3 rounded-xl border border-indigo-500/10 bg-indigo-500/[0.03] p-4">
                    <input wire:model="is_active" type="checkbox" id="spp_is_active"
                        class="mt-0.5 h-5 w-5 cursor-pointer rounded border-indigo-500/30 bg-white/50 text-indigo-600 focus:ring-indigo-500 dark:bg-white/10">
                    <div>
                        <x-ui.label for="spp_is_active" value="Tampilkan sebagai biaya aktif" class="cursor-pointer txt-primary" />
                        <p class="mt-1 text-xs txt-muted">Jika aktif dan tahun ajarannya sedang berjalan, biaya akan tampil pada halaman PPDB.</p>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-indigo-500/10">
                    ...
                    <x-ui.button variant="primary" type="submit">
                        {{ $editId ? 'Simpan Perubahan' : 'Tambah Biaya' }}
                    </x-ui.button>
                </div>
            </form>
        </div>
