<div style="display:flex;flex-direction:column;gap:24px;height:100%;" >
    ...
    <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <p class="txt-muted" style="font-size:13px;margin-top:4px;">Rekap penerimaan SPP, tunggakan siswa, dan
                efektivitas penagihan.</p>
        </div>
...
for ($y = now()->year; $y >= now()->year - 4; $y--) {
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="rounded-2xl p-5 flex flex-col gap-2"
            ...
            <p class="text-xl font-black text-emerald-400 leading-tight">Rp
                {{ number_format($totalHariIni, 0, ',', '.') }}</p>
        ...
        <div class="rounded-2xl p-5 flex flex-col gap-2"
            ...
            <p class="text-xl font-black text-indigo-400 leading-tight">Rp
                {{ number_format($totalBulanIni, 0, ',', '.') }}</p>
            <p class="text-xs txt-muted">
                {{ now()->format('m') == 1 ? 'Januari' : (now()->format('m') == 2 ? 'Februari' : (now()->format('m') == 3 ? 'Maret' : (now()->format('m') == 4 ? 'April' : (now()->format('m') == 5 ? 'Mei' : (now()->format('m') == 6 ? 'Juni' : (now()->format('m') == 7 ? 'Juli' : (now()->format('m') == 8 ? 'Agustus' : (now()->format('m') == 9 ? 'September' : (now()->format('m') == 10 ? 'Oktober' : (now()->format('m') == 11 ? 'November' : 'Desember')))))))))) }}
                {{ now()->format('Y') }}</p>
        </div>
        ...
        <div class="rounded-2xl p-5 flex flex-col gap-2"
            ...
            <p class="text-xl font-black text-purple-400 leading-tight">Rp
                {{ number_format($totalTahunIni, 0, ',', '.') }}</p>
