<div style="display:flex;flex-direction:column;gap:20px;">
    ...
    <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="txt-primary" style="font-size:24px;font-weight:800;letter-spacing:-0.02em;">Transaksi Pembayaran
            </h1>
            <p class="txt-muted" style="font-size:13px;margin-top:4px;">Catat pembayaran SPP siswa dengan cepat dan
                akurat.</p>
        </div>
        <div class="flex items-center gap-3">
            <span class="text-xs font-semibold txt-muted uppercase tracking-wider whitespace-nowrap">Tahun
                Tagihan:</span>
            <div class="w-32">
                @php
                    $yearOptions = [];
                ...
                @endphp
                <x-ui.select wire:model.live="selectedTahun" :options="$yearOptions" />
            </div>
        </div>
    </div>
    ...
    @if (!$selectedSiswaId)
        <x-ui.card class="overflow-visible" style="position: relative; z-index: 10;">
            ...
            <div class="relative" style="z-index: 99;">
                <input wire:model.live.debounce.250ms="searchQuery" type="text" id="search-siswa-input"
                    placeholder="Contoh: Ahmad Fauzi / 0012345678 ..." autocomplete="off"
                    ...
                    <div class="absolute top-full left-0 right-0 glass bg-white dark:bg-slate-900 mt-2 rounded-xl overflow-hidden shadow-2xl"
                        ...
                        @foreach ($this->hasilPencarian as $siswa)
                            <button wire:click="selectSiswa({{ $siswa['id'] }})" type="button"
                                ...
                                <div
                                    class="w-9 h-9 rounded-full bg-gradient-to-br
                        ...
                        flex items-center justify-center text-white text-sm font-bold flex-shrink-0">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-xs txt-muted truncate">{{ $siswa['nisn'] }} · {{ $siswa['kelas'] }}
                                    </p>
                                </div>
                                <span
                ...
                @elseif($showDropdown && strlen($searchQuery) >= 2)
                    <div class="absolute top-full left-0 right-0 glass bg-white dark:bg-slate-900 mt-2 rounded-xl overflow-hidden shadow-2xl px-4 py-6 text-center"
                        style="z-index: 9999;  border: 1px solid rgba(99,102,241,0.2);">
                        <p class="text-sm txt-muted">Siswa tidak ditemukan untuk "<strong>{{ $searchQuery }}</strong>"
                        </p>
                    </div>
                @endif
            </div>
            ...
            <div class="rounded-2xl overflow-hidden"
                ...
                <div class="px-5 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div>
                        ...
                        <p
                            class="text-2xl font-black {{ $totalBayar > 0 ? 'text-indigo-400' : 'txt-muted' }} transition-all duration-300">
                    ...
                    <div class="flex items-center gap-2.5 flex-wrap sm:flex-nowrap">
                        ...
                        {{-- Proses Bayar --}}
                        <button wire:click="prosesBayar" type="button" wire:loading.attr="disabled"
                            @class([
                                ...
                                'bg-gradient-to-r from-indigo-500 to-purple-600 text-white hover:from-indigo-600 hover:to-purple-700 hover:shadow-indigo-500/30' =>
                                    count($selectedItems) > 0,
                                'bg-slate-500/10 txt-muted cursor-not-allowed border border-slate-500/20' =>
                                    count($selectedItems) === 0,
            ...
            <x-ui.card>
                ...
                @if ($data['is_bulanan'])
                    ...
                    <div class="space-y-3">
                        {{-- Semester 1 --}}
                        <div>
                            <p class="text-[10px] font-bold txt-muted uppercase tracking-wider mb-2">Semester 1 —
                                Januari s/d Juni</p>
                                    ...
                                    <button
                                        ...
                                        type="button" @class([
                                            ...
                                            'bg-emerald-500/10 border-emerald-500/30 cursor-default' =>
                                                $bulan['lunas'] && !$isSebagian,
                                            'bg-amber-500/10 border-amber-500/30 cursor-default' =>
                                                $isSebagian && !$bulan['lunas'],
                                            'bg-indigo-500/20 border-indigo-500 ring-2 ring-indigo-500/30 cursor-pointer scale-95' =>
                                                !$bulan['lunas'] && $isSelected,
                                            'bg-red-500/8 border-red-500/25 hover:bg-red-500/15 hover:scale-95 cursor-pointer' =>
                                                !$bulan['lunas'] && !$isSelected && !$isFuture && !$isSebagian,
                                        ...
                                        @if ($bulan['lunas'] && !$isSebagian)
                                            <svg class="w-3 h-3 text-emerald-400 mt-0.5" fill="currentColor"
                                                viewBox="0 0 20 20">
                                        ...
                                        @elseif($isSebagian && !$bulan['lunas'])
                                            <span
                                                class="text-[8px] text-amber-400 mt-0.5 font-bold leading-none">{{ $bulan['persentase'] }}%</span>
                                        @elseif($isSelected)
                                            <svg class="w-3 h-3 text-indigo-300 mt-0.5" fill="none"
                                        ...
                                        @elseif(!$isFuture)
                                            <span
                                                class="text-[9px] text-red-400 mt-0.5 font-semibold leading-none">TUNGGAK</span>
                                        @else
                        ...
                        {{-- Semester 2 --}}
                        <div>
                            <p class="text-[10px] font-bold txt-muted uppercase tracking-wider mb-2">Semester 2 — Juli
                                s/d Desember</p>
                            <div class="grid grid-cols-6 gap-2">
                                @for ($b = 7; $b <= 12; $b++)
                                    @php
                ...
                @else
                    ...
                    @php
                        ...
                        $sisa = $data['sisa'] ?? 0;
                    @endphp
                    <button wire:click="{{ !$data['lunas'] ? 'toggleItem(' . $sppId . ', \'sekali\')' : '' }}"
                        type="button" @class([
                            'w-full flex items-center justify-between px-5 py-4 rounded-xl border-2 transition-all duration-150',
                            'bg-emerald-500/8 border-emerald-500/30 cursor-default' =>
                                $data['lunas'] && !$isSebagian,
                            'bg-amber-500/8 border-amber-500/30 cursor-default' =>
                                $isSebagian && !$data['lunas'],
                            'bg-indigo-500/15 border-indigo-500 cursor-pointer' =>
                                !$data['lunas'] && $isSelected,
                            'bg-red-500/6 border-red-500/20 hover:bg-red-500/12 cursor-pointer' =>
                                !$data['lunas'] && !$isSelected && !$isSebagian,
                        ])>
                        <div class="flex items-center gap-3">
                            @if ($data['lunas'] && !$isSebagian)
                                ...
                                <div>
                                    ...
                                    <p class="text-xs txt-muted">Dibayar: {{ $data['pembayaran_terakhir'] ?? '-' }}
                                    </p>
                                </div>
                            ...
                            @elseif($isSelected)
                                ...
                                <p class="text-sm font-bold text-indigo-300">Dipilih untuk dibayar — klik lagi untuk
                                    batal</p>
                            @else
        ...
        <x-ui.card>
            ...
            <div class="space-y-2">
                ...
                @foreach ($riwayat as $r)
                    <div
                        class="flex items-center gap-3 px-4 py-3 rounded-xl bg-white/[0.02] border border-white/[0.04] hover:bg-white/[0.04] transition-colors">
                        {{-- Icon kategori --}}
                        <div
                            class="w-8 h-8 rounded-lg bg-emerald-500/10 flex items-center justify-center flex-shrink-0">
                            <svg class="w-4 h-4 text-emerald-400" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd"
                                    d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
                                    clip-rule="evenodd" />
                        ...
                        {{-- Cetak Kuitansi --}}
                        <a href="{{ route('admin.keuangan.kuitansi.cetak', ['ids' => $r->id]) }}" target="_blank"
